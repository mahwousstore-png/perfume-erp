...
def light_normalize(name: str) -> str:
    ...
    # تطبيع كلمات شائعة (بحدود كلمة لتجنب التداخل)
    # إصلاح: "مل" تُحوّل فقط بعد أرقام (لا داخل "الملوك" أو "مليون")
    n = re.sub(r'(\d+(?:\.\d+)?)\s*مل\b', r'\1 ml', n)
    n = re.sub(r'\bملي\b', 'ml', n)
    
    # إصلاح: "عطر" تُزال فقط ككلمة مستقلة (لا داخل "عطري" أو "عطرلطافة")
    n = re.sub(r'\bعطر\b', '', n)
    
    word_replacements = {
        'تستر': 'tester', 'تيستر': 'tester',
        'عينة': 'sample',
...
class SmartMatcher:
    ...
    def _verify_match(
        ...
        # 4. تحقق إضافي: اسم المنتج بدون الماركة (أو الاسم الكامل إذا الماركة غير معروفة)
        # هذا يمنع مطابقة منتجات مختلفة من نفس الماركة أو بأسماء متشابهة
        my_product_name = self._remove_brand_from_name(my_light, my_brand) if my_brand else my_light
        comp_product_name = self._remove_brand_from_name(comp_light, comp_brand) if comp_brand else comp_light
        
        # إزالة كلمات التركيز والحجم حتى بدون ماركة
        if not my_brand:
            my_product_name = self._remove_brand_from_name(my_product_name, "")
        if not comp_brand:
            comp_product_name = self._remove_brand_from_name(comp_product_name, "")
        ...
        if my_product_name and comp_product_name:
            ...
            # إذا اسم المنتج مختلف جداً
            # التهجئة العربية المختلفة (جيرلان/غيرلان) تعطي word_overlap منخفض لكن product_score عالي
            # لذلك نستخدم معايير مختلفة حسب product_score
            if product_score < 60:
                return (False, name_score * 0.3, f"منتج مختلف (prod:{product_score}% words:{word_overlap:.0%})")
            
            if product_score < 75 and word_overlap < 0.4:
                return (False, name_score * 0.3, f"منتج مختلف (prod:{product_score}% words:{word_overlap:.0%})")
            
            # إذا product_score >= 75 لكن word_overlap منخفض، نستخدم fuzz.ratio للتأكد
            # (هذا يتعامل مع التهجئة المختلفة مثل جيرلان/غيرلان)
            if product_score >= 75 and word_overlap < 0.3:
                # تحقق إضافي: مقارنة حرفية (fuzz.ratio) للتعامل مع التهجئة
                char_ratio = fuzz.ratio(my_product_name, comp_product_name)
                if char_ratio < 65:
                    return (False, name_score * 0.3, f"منتج مختلف (prod:{product_score}% words:{word_overlap:.0%} char:{char_ratio}%)")
            
            # تعديل النتيجة بناءً على تشابه اسم المنتج
            adjusted_score = (name_score * 0.4) + (product_score * 0.4) + (word_overlap * 100 * 0.2)
            return (adjusted_score >= 78, adjusted_score, f"Score: {adjusted_score:.0f}% (product: {product_score}% words: {word_overlap:.0%})")
...
def run_smart_matching(
    ...
    for idx, my_p in enumerate(my_products):
        ...
        # 3. بحث fuzzy موسّع إذا لم نجد مرشحين كافيين
        if len(candidate_list) < 10:
            my_light = light_normalize(my_name)
            # بحث بكل الكلمات المميزة (بدون كلمات التركيز)
            ...
            for entry in all_candidates:
                if entry["index"] not in seen:
                    ...
                    if len(common) >= 1:  # كلمة مميزة واحدة على الأقل
                        quick = fuzz.token_set_ratio(my_light, entry["light"])
                        if quick >= 55:
                            seen.add(entry["index"])
                            candidate_list.append(entry)
                            if len(candidate_list) >= 50:  # حد أقصى أعلى
                                break
